...
impl<'a, U: Copy + 'a, T: Iterator<Item=&'a U>>  QuadFoldingIterator<'a, U,T> {
	...
	return self.two_line_buffer_index >= self.two_line_buffer.len();
    }
	...
	let actual_index = self.two_line_buffer_index - (on_quad + line) * 2 + self.line_length*line;
	self.two_line_buffer_index += 1;
	return self.two_line_buffer[actual_index];
    }
}

impl<'a, U: Default + Copy + 'a, T: Iterator<Item=&'a U>> Iterator for QuadFoldingIterator<'a, U,T> {
    type Item = U;
	...
	return Some(self.get_next_block());
    }
}
